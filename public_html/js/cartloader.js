document.addEventListener("DOMContentLoaded", async function () {
  ...
  const FAVORITES_VIEW_URL = "./user/index.html?view=favorites";
  ...
  relocateBuyCardIfNeeded();
  window.addEventListener("resize", relocateBuyCardIfNeeded);
  loadFavoriteState();
  renderDetails(product);
  initShareButtons();

  function relocateBuyCardIfNeeded() {
    const buyCard = document.querySelector(".buy-card");
    if (!buyCard) return;

    const isMobile = window.innerWidth <= 1200;
    const isInBody = buyCard.parentElement === document.body;

    if (isMobile && !isInBody) {
      document.body.appendChild(buyCard);
    } else if (!isMobile && isInBody) {
      const leftCol = document.querySelector(".left-col");
      if (leftCol) leftCol.appendChild(buyCard);
    }
  }
...
<div class="btn-cart-wrap">
    ...
    <div class="btn-cart-wrap_top">
         ...
         <div class="cart-status-text">
            ...
            <a href="./basket.html">مشاهده سبد خرید</a>
        </div>
